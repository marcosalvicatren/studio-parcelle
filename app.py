...
def page_new_report():
    ...
    with c2:
        ...
        year = st.selectbox("Anno *", range(cy+5, cy-10, -1))
    with c3:
...
def _report_editor(report: FeeReport, pl: PriceList):
    ...
    with tab_prest:
        ...
        for line in report.lines:
            ...
            compiled_total = sum(v for v in [line.effective_q1, line.effective_q2,
                                             line.effective_q3, line.effective_q4] if v > 0)
            total_lbl = fmt_currency(compiled_total) if compiled_total else "non compilato"
            ...
            with st.expander(f"{badge} **{line.service_code_snap}** — {line.description_snap}  |  {total_lbl}"):
                ...
                with st.form(f"line_{line.id}"):
                    ...
                    lbl = driver_lbl + (f" ({driver_unit})" if driver_unit else "")

                    # Tutti e 4 i trimestri hanno il campo driver
                    c1, c2, c3, c4 = st.columns(4)
                    dvs = {}; ovs = {}
                    ...
                    tc1.markdown("**Totali calcolati:**")
                    tc2.markdown(f"T1: **{fmt_currency(line.effective_q1)}**")
                    tc3.markdown(f"T2: **{fmt_currency(line.effective_q2)}**")
                    tc4.markdown(f"T3: **{fmt_currency(line.effective_q3)}**")
                    tc5.markdown(f"T4: **{fmt_currency(line.effective_q4)}**")
                    st.markdown(f"**Totale annuale: {fmt_currency(line.total)}**")

                    # Conguaglio: mostrato solo per scaglioni, calcolato al salvataggio
                    if is_scaglioni:
                        cong_val = getattr(line, "fee_q4") or 0.0
                        st.info(f"**Conguaglio annuale** (calcolato dopo salvataggio T4): {fmt_currency(cong_val)}")
    ...
    with tab_kpi:
        ...
        with st.form("kpi_form"):
            # Ore lavorate
            st.markdown("**Ore lavorate:**")
            kc1, kc2, kc3, kc4 = st.columns(4)
            ore_q1 = kc1.number_input("T1", value=float(report.ore_q1 or 0), step=0.25, key="ore1")
            ore_q2 = kc2.number_input("T2", value=float(report.ore_q2 or 0), step=0.25, key="ore2")
            ore_q3 = kc3.number_input("T3", value=float(report.ore_q3 or 0), step=0.25, key="ore3")
            ore_q4 = kc4.number_input("T4", value=float(report.ore_q4 or 0), step=0.25, key="ore4")

            # Compenso a ore
            st.markdown("**Compenso a ore (€):**")
            kd1, kd2, kd3, kd4 = st.columns(4)
            co_q1 = kd1.number_input("T1 ", value=float(report.comp_ore_q1 or 0), step=1.0, key="co1")
            co_q2 = kd2.number_input("T2 ", value=float(report.comp_ore_q2 or 0), step=1.0, key="co2")
            co_q3 = kd3.number_input("T3 ", value=float(report.comp_ore_q3 or 0), step=1.0, key="co3")
            co_q4 = kd4.number_input("T4 ", value=float(report.comp_ore_q4 or 0), step=1.0, key="co4")
    ...
    with tab_vista:
        ...
        st.markdown("---")
        tc0,tc1,tc2,tc3,tc4,tc5 = st.columns(6)
        tc0.markdown("**TOTALE**")
        for col, (k, v) in zip([tc1,tc2,tc3,tc4,tc5], totali.items()):
            col.metric(str(k), fmt_currency(v))
...
def page_settings():
    ...
    today = dt.date.today()
    # Mostra promemoria a fine trimestre (marzo, giugno, settembre, dicembre)
    if today.month in (3, 6, 9, 12) and today.day >= 25:
        st.warning(f"⚠️ **Promemoria:** Siamo a fine trimestre ({today.strftime('%d/%m/%Y')}). Ricordati di esportare il database prima di qualsiasi aggiornamento del software.", icon="💾")
    ...
    with t2:
        ...
        st.caption("Esporta il database prima di ogni aggiornamento del codice. Importalo dopo il deploy per ripristinare i dati.")
        ...
        with col_exp:
            ...
            if db_path.exists():
                with open(db_path, "rb") as f_db:
                    db_bytes = f_db.read()
                import datetime as dt
                fname = f"backup_studio_parcelle_{dt.date.today().strftime('%Y%m%d')}.db"
                st.download_button(
                    "⬇️ Esporta database",
                    data=db_bytes,
                    file_name=fname,
                ...
                st.info("Database non trovato.")

        with col_imp:
            up_db = st.file_uploader("⬆️ Importa database (.db)", type=["db"])
            if up_db:
                if st.button("🔄 Ripristina database", type="primary", use_container_width=True):
                    with open(db_path, "wb") as f_db:
                        f_db.write(up_db.read())
